module Mutations
  class RegisterAttendance < BaseMutation
    ...
    def resolve
      ...
      purchase.update(remaining_visits: purchase.remaining_visits - 1).tap do |success|
        Visit.create!(purchase: purchase, visited_at: Time.current) if success
      end ? { success: true, purchase: purchase, errors: nil } : { success: false, purchase: nil, errors: purchase.errors.full_messages }
    end
  end
end
